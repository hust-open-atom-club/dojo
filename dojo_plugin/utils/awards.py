...
from dojo_plugin.utils.kook import get_kook_user, send_message
...
def update_awards(user):
    ...
    kook_user = asyncio.run(get_kook_user(user.id))
    if kook_user:
        for belt in BELT_REQUIREMENTS:
            ...
            asyncio.run(send_message(message, "belting-ceremony"))
